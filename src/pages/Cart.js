...
const Cart = () => {
    ...
    const numberOfItems = useSelector(state => state.cart.items.reduce((sum, e) => sum + e.count, 0));

    const pizzas = useSelector(state => state.cart.items);
    const totalPrice = useSelector(state => state.cart.totalPrice);
    return (
        <div className="container container--cart">
            {pizzas.length === 0 ? <CartEmpty/> : <div className="cart">
                ...
                <div className="content__items">
                    {pizzas.map(e => (
                        <CartPizzaBlock key={e.id} {...e}
                        />
                    ))}
                </div>
                <div className="cart__bottom">
                    <div className="cart__bottom-details">
                        <span> Всего пицц: <b>{numberOfItems} шт.</b> </span>
